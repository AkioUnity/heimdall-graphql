...
export class Utils {
   ...
   * @return {Boolean}
   */
  public verifySignature(signature, data, publicKey) {
   ...
   */
  public randomBase64URLBuffer(len?) {
    len = len || 32;

    let buff = crypto.randomBytes(len);

    let buffStringify = JSON.stringify(buff);

    let buffParse = JSON.parse(buffStringify);

    let buffDataStringify = JSON.stringify(buffParse.data);

    return base64url.encode(buffDataStringify);
   ...
   */
  public generateServerGetAssertion(authenticators) {
   ...
   */
  public COSEECDHAtoPKCS(COSEPublicKey) {
    /* 
       +------+-------+-------+---------+----------------------------------+
   ...
   */
  public parseMakeCredAuthData(buffer) {
  ...
  public verifyAuthenticatorAttestationResponse(webAuthnResponse) {
    let attestationBuffer = base64url.toBuffer(webAuthnResponse.response.attestationObject);
    let ctapMakeCredResp = cbor.decodeAllSync(attestationBuffer)[0];

    let response: any = { 'verified': false };
    console.log('ctapMakeCredResp: ', ctapMakeCredResp);
    if (ctapMakeCredResp.fmt === 'fido-u2f' || ctapMakeCredResp.fmt === 'android-safetynet') {
      let authrDataStruct = this.parseMakeCredAuthData(ctapMakeCredResp.authData);

      if (!(authrDataStruct.flags & this.U2F_USER_PRESENTED))
        throw new Error('User was NOT presented durring authentication!');

      let clientDataHash = this.hash(base64url.toBuffer(webAuthnResponse.response.clientDataJSON))
      let reservedByte = Buffer.from([0x00]);
      let publicKey = this.COSEECDHAtoPKCS(authrDataStruct.COSEPublicKey)
      let signatureBase = Buffer.concat([reservedByte, authrDataStruct.rpIdHash, clientDataHash, authrDataStruct.credID, publicKey]);

      let PEMCertificate = this.ASN1toPEM(ctapMakeCredResp.attStmt.x5c[0]);
      ...
      let PEMCertificate = new Buffer(this.ASN1toPEM(ctapMakeCredResp.attStmt.x5c[0])); // customized by me
      let signature = ctapMakeCredResp.attStmt.sig;
   ...
   */
  public findAuthr(credID, authenticators) {
    for (let authr of authenticators) {
   ...
   * @return {Object}        - parsed authenticatorData struct
   */
  public parseGetAssertAuthData(buffer) {
    let rpIdHash = buffer.slice(0, 32); buffer = buffer.slice(32);
    let flagsBuf = buffer.slice(0, 1); buffer = buffer.slice(1);
    let flags = flagsBuf[0];
    let counterBuf = buffer.slice(0, 4); buffer = buffer.slice(4);
  ...
  public verifyAuthenticatorAssertionResponse = (webAuthnResponse, authenticators) => {
    let authr = this.findAuthr(webAuthnResponse.id, authenticators);
    ...
    if (authr.fmt === 'fido-u2f') {
      ...
      let publicKey = this.ASN1toPEM(base64url.toBuffer(authr.publicKey));
